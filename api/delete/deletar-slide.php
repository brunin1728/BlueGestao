...
if($USER->SENHA === $SENHA2){
	...
	if ( $validate ) {
		$stmt = $conn->prepare
		("UPDATE tbl_slide SET STATUS_CONT = '10' WHERE ID = '".$ID_S."'");
		
	

		if ( $stmt->execute() ) {
			 //header(sprintf('location: %s', $_SERVER['HTTP_REFERER'].'&sucesso=1'));
			 header("Location: ../../paginas.php?action=page&i=".$ID_P."&op=slide&sucesso=1");
		} else {
    ...
    header("Location: ../../paginas.php?action=page&i=".$ID_P."&op=slide&serro=1");
}
